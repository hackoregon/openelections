...
class SearchBox extends Component {
    ...
    onUpDownNavigation ( e ) {
        ...
        if ( e.keyCode === 40 && !withModifierKey ) {
            const newSelectedItem = Math.min( this.guesses.length - 1, this.state.selectedItem + 1 );
          ...
          // Enter Key
            ...
            this.selectGuessItem ( this.guesses[this.state.selectedItem] );
            e.target.blur();
        }
    }
    ...
    onMouseDownOverGuessItem ( e ) {
        ...
        this.selectGuessItem( this.guesses[itemIndex] );
    }
    ...
    renderGuessesList ( guesses ) {
        this.renderedGuesses = guesses.map( (item, i)=>
            <li key={i}
                data-index={i}
                className={'guess-item' + ((i === this.state.selectedItem) ? ' focused' : '')}
                onMouseEnter={this.onHoverOverGuessItem}
                onMouseDown={this.onMouseDownOverGuessItem}
            >
                <a href={'javascript:void'}><span className={'item-label'}>{item.label}</span>{item.value}</a>
            </li>
        );

        return this.renderedGuesses;
    }

    render() {
        const guessesList = this.renderGuessesList(this.guesses);

        return (
            <div css={this.styles(this.props)}>
